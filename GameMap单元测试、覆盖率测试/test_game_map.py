...
class TestGameMap(unittest.TestCase):
    ...
    def test_init(self):
        self.assertRaises(TypeError, GameMap, (1.2,4))
        self.assertRaises(TypeError, GameMap, (4,1.3))
    ...
    def test_reset_get_set(self):
        self.assertRaises(TypeError,self.game_map.reset,(4))
        self.game_map.reset()
...
def main():
    filepath = 'F:/multisim/VsCode/软件工程学习/test_game_map_report.html'
    ftp=open(filepath,'wb')

    suite = unittest.makeSuite(TestGameMap)
    
    # unittest.main(verbosity=2)
    runner=HTMLTestRunner.HTMLTestRunner(stream=ftp,title='test_game_map')
    runner.run(suite)
